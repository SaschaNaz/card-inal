...
function parseAudio(head) {
    const card = Object.assign({}, parsePlayer(head), { card: "audio" });
...
function parseUndocumented(head, card) {
    return Object.assign({ card, undocumented: true }, getAllMeta(head));
}
...
function getMetaMap(head) {
    const metas = head.querySelectorAll("meta[name^='twitter:'],meta[property^='twitter:']");
    const map = new Map();
    for (const meta of Array.from(metas)) {
...
function setRecursive(object, propertyPath, value) {
